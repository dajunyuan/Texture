...
void Mesh_Painter::update_vertex_buffer()
{
	...
	for (unsigned int m_i = 0; m_i < this->m_my_meshes_.size(); m_i++)
	{
		...
		{
			int index = face_list[3 * i].x; //x为顶点坐标索引
			// 与4.1不同的是，这里将face_list改成vec3的类型，并且不是分别存取v v v, vt vt vt, vn vn vn
			// 而是v vt vn, ...
			// 需要做出较大修改
			points[3 * i] = vec3(
		...
		for (int i = 0; i < num_face; i++)
		{
			int index = face_list[3 * i].z;  //z为法线坐标索引
			points[3 * i] = vec3(
		...
		for (int i = 0; i < num_face; i++)
		{
			int index = face_list[3 * i].y;    //y为纹理坐标索引
			points_2[i * 3] = vec2(vt_list[index * 3 + 0], vt_list[index * 3 + 1]);
			index = face_list[3 * i + 1].y;
			points_2[i * 3 + 1] = vec2(vt_list[index * 3 + 0], vt_list[index * 3 + 1]);
			index = face_list[3 * i + 1].y;
			points_2[i * 3 + 2] = vec2(vt_list[index * 3 + 0], vt_list[index * 3 + 1]);
		}
